class DiagnosisUnitsController < ApplicationController
  ...
  def show
    ...
    id: @diagnosis_unit.health_unit_id
  end
  ...
  def basic_search
    if params[:keywords].empty?
      redirect_to diagnosis_unit_path
    else
  ...
  def list_by_specialties
    if params[:specialty].nil?
      ...
      @diagnosis_units = DiagnosisUnit.where("specialties && ARRAY[?]",
        @specialty)
      respond_to do |format|
  ...
  def list_by_treatments
    if params[:treatments].empty?
      ...
      @diagnosis_units = DiagnosisUnit.where("treatments && ?",
